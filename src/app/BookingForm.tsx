...
export default function BookingForm({ selectedSlot, onFormSubmit, onCancel, duration }: BookingFormProps) {
  ...
  return (
    <div className="w-full max-w-md p-8 bg-gray-100 rounded-3xl shadow-lg">
      <h2 className="text-2xl font-bold text-gray-800 text-center mb-2">Confirmar Cita</h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          ...
          <input type="text" id="name" name="name" required className="w-full px-3 py-2 text-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:border-blue-500" />
        </div>
        <div className="mb-4">
          <label htmlFor="email" className="block text-gray-700 font-semibold mb-2">Correo Electrónico</label>
          <input type="email" id="email" name="email" required className="w-full px-3 py-2 text-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:border-blue-500" />
        </div>
        <div className="mb-6">
          <label htmlFor="notes" className="block text-gray-700 font-semibold mb-2">Motivo de la llamada (Opcional)</label>
          <textarea id="notes" name="notes" rows={3} className="w-full px-3 py-2 text-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:border-blue-500"></textarea>
        </div>
        <div className="flex justify-between items-center">
          <button type="button" onClick={onCancel} className="text-gray-600 hover:text-gray-800">Cancelar</button>
          <button type="submit" className="bg-blue-600 text-white font-bold py-2 px-6 rounded-3xl hover:bg-blue-700">Confirmar Cita</button>
        </div>
      </form>
    </div>
  );
